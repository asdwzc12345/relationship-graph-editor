...
const requiredSources = ["BackgroundWork.cs", "GraphCanvas.cs", "GraphHistory.cs", "GraphLayout.cs", "GraphModel.cs", "MainForm.cs", "NativeAppIcon.cs", "NativeDrawioImport.cs", "NativeExport.cs", "NativePdfExport.cs", "NativePersistence.cs", "NativeTheme.cs", "Program.cs", "RecentFileStore.cs"];
...
assert.match(sourceText, /class NativeDrawioImport/);
assert.match(sourceText, /DtdProcessing\.Prohibit/);
assert.match(sourceText, /relationship-graph-v1/);
assert.match(mainFormText, /\*\.drawio/);
assert.match(mainFormText, /导入 JSON \/ Draw\.io \/ 只读可视图/);
assert.match(sourceText, /GraphLayout\.CalculateRoutes/);
assert.match(sourceText, /PortSpacing/);
assert.match(sourceText, /HasObstacleConflict/);
assert.match(sourceText, /LabelBounds/);
assert.match(graphCanvasText, /DrawMiniMap/);
...
assert.match(mainFormText, /DrawioPageDialog/);
assert.match(mainFormText, /StartBackgroundWork/);
assert.match(mainFormText, /PublishPreparedExport/);
assert.match(sourceText, /class BackgroundWorkQueue/);
assert.match(sourceText, /class DebouncedSaveQueue/);
assert.match(sourceText, /class RecentFileStore/);
assert.match(mainFormText, /MenuItem\("新增节点", Keys\.Insert/);
assert.doesNotMatch(mainFormText, /MenuItem\("新增节点", Keys\.Control \| Keys\.Shift \| Keys\.N/);
assert.match(sourceText, /AppendSvgNodeShape/);
assert.match(sourceText, /NodeShapePath/);
assert.match(sourceText, /value == "auto"/);
assert.match(mainFormText, /自动避障/);
assert.doesNotMatch(sourceText, /static readonly JavaScriptSerializer/);
assert.match(sourceText, /beforePublish/);
assert.match(sourceText, /CancellationRequested/);
